...
function run_heat_2d_periodic(N_local, α, CFL, L, nt)
    MPI.Init()
    comm = MPI.COMM_WORLD
    ...
    #u= HaloArray(Float64,N_local,h,topology; boundary_condition = (:repeating, :repeating))
    u= HaloArray(Float64,N_local,h,topology; boundary_condition = (:periodic, :periodic))
...
run_heat_2d_periodic(N_local, alpha, CFL, L, nt)
...
using Plots
plot(dset[1,:,50], label="t=0")
plot!(dset2[1,:,50], label="t=0")
plot!(dset[100,:,50], label="t=1")
plot!(dset2[100,:,50], label="t=1")
