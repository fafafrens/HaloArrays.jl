...
function make_random_buffers(phi)
    ...
    region0 = interior_cell_window(ranges, 0; compressed_dim = 2)
...
function phi4_sweep!(kernel!, backend, phi, params, buffers::RandomBuffers)
    ...
    for color in 0:1
        ...
        # Ghost-fill broadcasts, Metal.rand! and the kernel all share the backend
        # queue, so they execute in order — no host sync needed inside the sweep.
