...
using HaloArrays:
    ...
    halo_width, interior_view, owned_size, interior_size, storage_size,
    interior_range, get_send_view, get_recv_view,
    make_recv_buffers, make_send_buffers, validate_boundary_condition,
    tag_send, tag_recv,
    normalize_boundary_condition, infer_periodicity,
...
function HaloArrays.inactive_cartesian_topology(dims::NTuple{N,<:Integer}) where {N}
...
HaloArrays.inactive_cartesian_topology(n::Int) =
    HaloArrays.inactive_cartesian_topology(ntuple(_ -> 0, n))

HaloArrays.inactive_cartesian_topology(::Val{N}) where {N} =
    HaloArrays.inactive_cartesian_topology(ntuple(_ -> 0, Val(N)))
...
function CartesianTopology(comm::MPI.Comm, dims::NTuple{N,<:Integer};
    ...
    (comm == MPI.COMM_NULL || !active) &&
        return HaloArrays.inactive_cartesian_topology(dims_int)
...
# is_root needs MPI.Comm_rank; override the field-based version from the main package
# (field global_rank is set correctly at construction, so the main-package version is fine)
...
# Undef constructor (for OrdinaryDiffEq workspace allocation)
function HaloArray{T,N,arraytype,Halo}(::UndefInitializer,
    ...
    topology   = HaloArrays.inactive_cartesian_topology(Val(N))
    comm_state = HaloCommState(N)
    recv_bufs  = make_recv_buffers(data, Halo)
    send_bufs  = make_send_buffers(data, Halo)
    bc = normalize_boundary_condition(boundary_condition, N)
...
# ============================================================
# LinearAlgebra.norm (needs MPI mapreduce)
# ============================================================
...
HaloArrays.halo_exchange!(halo::HaloArray) = halo_exchange_waitall_unsafe!(halo)

HaloArrays.start_halo_exchange!(halo::HaloArray) = start_halo_exchange_async_unsafe!(halo)
HaloArrays.finish_halo_exchange!(halo::HaloArray) = end_halo_exchange_async_wait_unsafe!(halo)

function HaloArrays.synchronize_halo!(halo::HaloArray)
    HaloArrays.halo_exchange!(halo)
...
function HaloArrays.gather_haloarray(halo::HaloArray; root::Int=0)
    comm    = halo.topology.cart_comm
    rank    = MPI.Comm_rank(comm)
    nproc   = MPI.Comm_size(comm)
    N       = HaloArrays.ndims(halo)
    T       = eltype(halo)
    ...
    recvbuf = rank == root ? Array{T}(undef, local_len * nproc) : nothing

    MPI.Gather!(sendbuf, recvbuf, comm; root=root)

    rank == root || return nothing

    global_sz    = ntuple(i -> dims[i] * owned_shape[i], Val(N))
    global_array = Array{T}(undef, global_sz)
    for r in 0:nproc-1
        ...
        flat_off   = r * local_len + 1
        subarray   = reshape(view(recvbuf, flat_off:flat_off+local_len-1), owned_shape...)
...
function Base.any(f::F, u::HaloArray) where {F<:Function}
    MPI.Allreduce(any(f, interior_view(u)) :: Bool, |, HaloArrays.get_comm(u))
end

function Base.all(f::F, u::HaloArray) where {F<:Function}
    MPI.Allreduce(all(f, interior_view(u)) :: Bool, &, HaloArrays.get_comm(u))
end

# ============================================================
# mapreduce_haloarray_dims / mapreduce_mhaloarray_dims
# ============================================================

function HaloArrays.mapreduce_haloarray_dims(f, op, ha::HaloArray{T,N,A,Halo}, dims) where {T,N,A,Halo}
    topo           = ha.topology
    root_coord     = 0
...
function HaloArrays.mapreduce_mhaloarray_dims(f, op, mha::MultiHaloArray, dims)
    names = keys(mha.arrays)
    list_of_maybe = HaloArrays.map_over_field(mha) do field
        HaloArrays.mapreduce_haloarray_dims(f, op, field, dims)
    end
